...
class TverskyFocalLoss(nn.Module):
    ...
    def forward(self, predict, target):
        ...
        if predict.shape == target.shape:
            ...
            raise ValueError("The shapes of 'predict' and 'target' are incompatible.")
        ...
        if self.weight is None:
            ...
            if isinstance(self.weight, list):
                self.weight = torch.tensor(self.weight, dtype=torch.float64).to(device)
...
class LocallyWeightedTverskyFocalLoss(nn.Module):
    r"""
    Tversky focal loss weighted by inverse of label frequency calculated locally based
    on the input batch.
    Arguments:
        ignore_index (int): Class index to ignore
        predict (torch.tensor): Predicted tensor of shape [N, C, *]
    ...
    def forward(self, predict, target):
        ...
        # Initialize the loss
        loss_fn = TverskyFocalLoss(weight=loss_weight, ignore_index=self.ignore_index, **self.kwargs)
...
class TverskyFocalCELoss(nn.Module):
    ...
    def __init__(self, loss_weight=None, tversky_weight=0.5, tversky_smooth=1, tversky_alpha=0.7, 
                 tversky_gamma=1.33, ignore_index=-100):
