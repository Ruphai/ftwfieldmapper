...
def do_normalization(img, normal_strategy, stat_procedure, nodata=[0, 65535], 
    ...
    Args:
        ...
        stat_procedure (str): Procedure to calculate the statistics used in normalization.
                              Options:
                                    ...
                                    - 'gpb': global per band.
        nodata (int): Values reserved to show nodata.
    ...
    img = np.where((img < 0) | np.isin(img, nodata), np.nan, img)
    # if np.all(np.isnan(img_tmp)):
    #     print('found nan image')
    #     print(img_tmp)
    #     print(np.nanmin(img_tmp))
    #     exit()
